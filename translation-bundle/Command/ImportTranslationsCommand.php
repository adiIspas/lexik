...
class ImportTranslationsCommand extends ContainerAwareCommand
{
    ...
    protected function configure()
    {
        ...
        $this->addOption('application-bundles', 'a', InputOption::VALUE_NONE, 'Search for translations in all application Bundles in src folder');
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        if ($bundleName) {
            ...
            if (!$this->input->getOption('globals') && !$this->input->getOption('application-bundles')) {
            ...
            if($this->input->getOption('application-bundles')) {
                $this->importBundlesTranslationFiles($locales, $domains, false);
            }
    ...
    {
        $basePath = $this->getApplication()->getKernel()->getRootDir();
        $basePath = substr($basePath,0,-3);
        $basePath .= 'src\\';

        $bundles = $this->getApplication()->getKernel()->getBundles();
        foreach ($bundles as $bundle) {
            $currentPath = $basePath .  $bundle->getName();
            $currentBundlePath = $bundle->getPath();

            if(!strcmp($currentPath,$currentBundlePath)) {
                $this->output->writeln('Bundle: ' . $currentPath . ' | ' . $currentBundlePath);
